...
// The DomainWhoisResult struct stores all of the available
...
// The DomainWhoisError struct stores errors
...
func OpenDomainWhois(config *Configuration) (*DomainWhois, error) {
	var whois = &DomainWhois{}
	whois.configuration = config
	whois.baseUrl = "api.ip2whois.com/v2"
	return whois, nil
}
...
func (a *DomainWhois) LookUp(domain string) (DomainWhoisResult, error) {
	var res DomainWhoisResult
	var ex DomainWhoisError

	myUrl := "https://" + a.baseUrl + "?key=" + url.QueryEscape(a.configuration.apiKey) + "&domain=" + url.QueryEscape(domain) + "&source=" + url.QueryEscape(a.configuration.source) + "&source_version=" + url.QueryEscape(a.configuration.sourceVersion)
